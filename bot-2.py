...
MAX_RAM_BYTES    = int(os.environ.get("MAX_RAM_MB", 1800)) * 1024 * 1024
...
try:
    ...
    print(f"✅ RAM limit: {MAX_RAM_BYTES // 1024 // 1024} MB")
except Exception as e:
    print(f"⚠️ Could not set RAM limit: {e}")
...
async def process_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await edit_or_reply(
            status_msg,
            "✅ تم التحميل.\n🎬 جارٍ استخراج الصوت…" if is_video
...
def main() -> None:
    if BOT_TOKEN == "YOUR_BOT_TOKEN_HERE":
        raise SystemExit("❌ ضع توكن البوت في متغير البيئة BOT_TOKEN.")
    ...
    print(f"🤖 البوت يعمل | CPU: {CPU_THREADS} threads | RAM: {MAX_RAM_BYTES // 1024 // 1024} MB | Timeout: {TELEGRAM_TIMEOUT}s")
